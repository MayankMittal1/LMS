...
router.get('/', function(req, res, next) {
    if(req.session.uid & req.session.type=="teacher" & !req.query.q){
      connection.query(
        `select * from teachers where id=${connection.escape(req.session.uid)};`,
...
router.post('/login', function(req, res, next) {
    connection.query(
      `select * from teachers where userid=${connection.escape(req.body.uid)} and password = "${crypto.createHash('sha256').update(req.body.password).digest('hex')}";`,
      (error, results, fields) => {
          console.log(results)
        if (error) {
          res.writeHead(500);
    ...
    if(req.session.uid & req.session.type=="teacher"){
        ...
        file.mv('public/images/uploads/'+file.name, function(err) {           
	        if (err) return res.status(500).send(err);
      		var sql = `insert into books(name,author,genre,quantity,image) values(${connection.escape(req.body.bookName)},${connection.escape(req.body.authorName)},${connection.escape(req.body.genre)},${connection.escape(req.body.quantity)},'${img_name}') ;`
...
router.post('/saveBook',(req,res)=>{
  if(req.session.uid & req.session.type=="teacher"){
    if(!req.files){
      var q=`update books set name=${connection.escape(req.body.bookName)} , author=${connection.escape(req.body.authorName)}, genre = ${connection.escape(req.body.genre)} , quantity=${connection.escape(req.body.quantity)} where id=${connection.escape(req.query.id)};`
      ...
      var img_name=file.name          
          file.mv('public/images/uploads/'+file.name, function(err) {           
            if (err) return res.status(500).send(err);
            var sql = `update books set name=${connection.escape(req.body.bookName)} , author=${connection.escape(req.body.authorName)}, genre = ${connection.escape(req.body.genre)} , quantity=${connection.escape(req.body.quantity)} , image="${img_name}" where id=${connection.escape(req.query.id)};`
            connection.query(sql, function(err, result) {
              console.log(result + err)
              res.redirect('/teacher')
              });
...
router.get('/delete',(req,res)=>{
  if(req.session.uid & req.session.type=="teacher"){
    var id=req.query.id
    connection.query(`delete from books where id = ${connection.escape(id)};`,(err,result)=>{
      if(err){
        res.end('error')
      }
      else{
...
router.post('/addStudent',(req,res)=>{
  if(req.session.uid & req.session.type=="teacher"){
    connection.query(`insert into students(name,phone,userid,password) values(${connection.escape(req.body.studentName)},${connection.escape(req.body.phone)},${connection.escape(req.body.userid)},"${crypto.createHash('sha256').update(req.body.password).digest('hex')}");`,(err,result)=>{
      res.redirect('/teacher')
    })
  }
  else{
...
router.get('/approve',(req,res)=>{
  if(req.session.type=="teacher"){
    ...
    connection.query(`update transaction set issue_date="${curDate}" , teacher_id=${connection.escape(req.session.uid)}, status="issued" where id=${id};`,(err,result)=>{
      if(err){
        console.log(err)
      }
      else{
